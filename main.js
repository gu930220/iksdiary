...
function templateHTML(title, body, button) {
  ...
  <section id="container">
      <header class="header">
          <h1 class="logo">
              <a href="/">ikmo's bulletin board.</a>
          </h1> 
      </header>
...
function templateList(filelist) {
  ...
  while (j < filelist.length) {
    list =
      list +
      `<li><a href="/?id=${filelist[j].id}">${filelist[j].id}&nbsp;&nbsp;${filelist[j].title}&nbsp;&nbsp;${filelist[j].created}</a></li>`;
    j = j + 1;
  }
  list = list + "<ul>";
  return list;
...
var app = http.createServer(function (request, response) {
  ...
  if (pathname === "/") {
    if (queryData.id === undefined) {
      ...
      db.query(
        `SELECT id, title, description, created FROM list`,
      ...
      db.query(`SELECT * FROM list`, function (error, results) {
        ...
        db.query(
          `SELECT * FROM list where id=${queryData.id}`,
    ...
    var html = templateHTML(
      ...
      `
        <form action="/create_process" method="post">
          ...
          <p>
            <textarea name="description" placeholder="description" style="width:500px; height:300px;"></textarea>
          </p>
          <p>
            <input type="submit">
          </p>
    ...
    db.query(`SELECT * FROM list`, function (error, results) {
      ...
      <form action="/update_process" method="post">
        ...
        <p>
          <textarea name="description" placeholder="description" style="width:500px; height:300px;">${result[0].description}</textarea>
        </p>
        <p>
          <input type="submit">
        </p>
